...
# Periksa apakah script dijalankan dengan hak superuser (root)
if [ "$(id -u)" != "0" ]; then
    echo "Skrip ini harus dijalankan dengan hak superuser."
    echo "Silakan coba beralih ke pengguna root menggunakan perintah 'sudo -i' lalu jalankan skrip ini kembali."
    exit 1
fi

# Periksa dan instalasi Node.js dan npm
...
function check_and_set_alias() {
    ...
    if ! grep -q "$alias_name" "$shell_rc"; then
        echo "Setel shortcut '$alias_name' ke $shell_rc"
        echo "alias $alias_name='bash $SCRIPT_PATH'" >> "$shell_rc"
        # Tambahkan pesan untuk mengaktifkan alias
        echo "Shortcut '$alias_name' sudah diatur. Jalankan 'source $shell_rc' untuk mengaktifkan alias, atau buka ulang terminal."
    else
        # Jika alias sudah diatur, berikan pesan informasi
        echo "Shortcut '$alias_name' sudah diatur di $shell_rc."
        echo "Jika shortcut tidak berfungsi, coba jalankan 'source $shell_rc' atau buka ulang terminal."
    fi
}
...
# Uninstall node Titan
function uninstall_node() {
    echo "Apakah Anda yakin ingin menghapus program node titan? Ini akan menghapus semua data terkait. [Y/Y]"
    read -r -p "Silakan konfirmasi: " response

    case "$response" in
        [yY][eE][sS]|[yY]) 
            echo "Memulai proses penghapusan program node..."
            pm2 stop titand && pm2 delete titand
            rm -rf $HOME/.titand $HOME/titan $(which titand)
            echo "Penghapusan program node selesai"
            ;;
        *)
            echo "Batalkan operasi penghapusan."
            ;;
    esac
}
...
function check_balances() {
    read -p "Masukkan alamat dompet: " wallet_address
    titand query bank balances "$wallet_address"
}

# Periksa status sinkronisasi node
...
function add_validator() {
    read -p "Masukkan nama dompet: " wallet_name
...
# Berdelegasi ke validator sendiri
function delegate_self_validator() {
read -p "Masukkan jumlah token yang distake: " math
read -p "Masukkan nama dompet: " wallet_name
titand tx staking delegate $(titand keys show $wallet_name --bech val -a)  ${math}art --from $wallet_name --fees 500uttnt

}

# Ekspor kunci priv validator
function export_priv_validator_key() {
    echo "====================Harap cadangkan semua konten berikut ke dalam notepad atau lembar Excel Anda==========================================="
...
function update_script() {
    ...
    chmod +x $SCRIPT_PATH
    echo "Skrip telah diperbarui. Silakan keluar dari skrip dan jalankan bash laodau.sh untuk menjalankan kembali skrip."
}
...
function main_menu() {
    while true; do
        ...
        echo "Silakan pilih operasi yang ingin Anda lakukan:"
        echo "1. Instalasi node"
        echo "2. Buat dompet"
        echo "3. Impor dompet"
        ...
        echo "11. Delegasikan ke diri sendiri" 
        ...
        echo "Tekan tombol apa pun untuk kembali ke menu utama..."
        read -n 1
    done
